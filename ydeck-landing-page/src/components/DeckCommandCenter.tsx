    ...
    <>
      ...
      <div className="relative z-10 mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_190px] xl:grid-cols-[minmax(0,1fr)_220px]">
        <div className="grid min-w-0 gap-4">
          <div className="rounded-2xl border border-white/10 bg-black/40 p-4 font-mono text-sm leading-6 text-slate-200">
            <TypingPrompt
                  ...
                  >
                    ...
                    <div className="h-px bg-white/10">
                      <motion.div
                        ...
                        animate={{ width: completed || active ? '100%' : '0%' }}
...
function MobileDeckCommandCenter({
  ...
  const reduceMotion = useReducedMotion();
  const activePreview = web3TemplateSlides[activeStep % web3TemplateSlides.length];
    ...
    <div className="glass-panel slide-card-shadow relative w-full overflow-hidden rounded-[1.5rem] p-3 shadow-[0_24px_80px_rgba(0,0,0,0.45)] lg:hidden">
      ...
      <div className="relative z-10 rounded-[1.25rem] border border-white/10 bg-black/35 p-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-ydeck-green/30 bg-ydeck-green/10 px-2.5 py-1 text-[10px] font-medium text-ydeck-green">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-ydeck-green" />
            <span className="max-w-[5.5rem] truncate">{content.status}</span>
          </div>
        </div>

        <div className="mt-3 rounded-2xl border border-white/10 bg-black/45 p-3 font-mono text-xs leading-5 text-slate-200">
          ...
          {content.timelineSteps.slice(0, 3).map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <span className={active ? 'text-xs text-white' : 'text-xs text-slate-500'}>
                  {step}
                </span>
              </div>
            );
